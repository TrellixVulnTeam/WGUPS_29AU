...
def package_status(HashDS, selected_time, package_ID):
    ...
    if get_time_value(package[6]) > get_time_value(selected_time):
        print(f'ID:{package[0]}, Address: {package[1]}, Deadline:{package[2]}, City:{package[3]}, ZipCode:{package[4]}, weight:{package[5]}, Truck:unassigned, Status:At Hub, Delivered:TBD')
    elif get_time_value(package[8]) > get_time_value(selected_time) >= get_time_value(package[6]):
        print(f'ID:{package[0]}, Address: {package[1]}, Deadline:{package[2]}, City:{package[3]}, ZipCode:{package[4]}, weight:{package[5]}, Truck:{package[7]}, Status:En Route, Delivered:TBD')
    elif get_time_value(selected_time) >= get_time_value(package[8]):
        print(f'ID:{package[0]}, Address: {package[1]}, Deadline:{package[2]}, City:{package[3]}, ZipCode:{package[4]}, weight:{package[5]}, Truck:{package[7]}, Status:Delivered, Delivered:{package[8]}')
